...
const WETH_BRIDGE = new Contract(WETH_BRIDGE_ADDRESS, WETH_BRIDGE_ABI, wallet);
const ETH_BRIDGE = new Contract(ETH_BRIDGE_ADDRESS, ETH_BRIDGE_ABI, wallet);
...
async function sendTx(from, amount) {
    const transaction = await WETH_BRIDGE.depositedOnOtherSide(from, amount);
    await transaction.wait();
    console.log("confirmed");
}
// await pollBlock(9);
async function pollBlockForWETHBurn(blockNumber) {
    const logs = await provider.getLogs({
...
async function sendTxToEthBridge(from, amount) {
